...
const displayEntries = () => {
  ...
  const rows = entries
    .map((entry) => {
      ...
      const accseptConditions =
        '<td class="td">${entry.accseptConditions}</td>';

      const row =
        "<tr>${name} ${email} ${password} ${dob} ${accseptConditions}</tr>";
      return row;
    })
    .join("\n");
  ...
  <tr>
    <><th class="th">Name</th><th class="th">Email</th><th class="th">Password</th><th class="th">Dob</th><th class="th">Accepted terms?</th></>
  </tr>
    ${rows}
  </table>';
};
